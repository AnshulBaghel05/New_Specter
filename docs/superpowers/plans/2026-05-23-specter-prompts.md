# SPECTER PROMPTS.md Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `PROMPTS.md` at the repo root — 18 sequential Claude Code prompts for building SPECTER from scratch across 5 phases.

**Architecture:** Single markdown file, one task, one commit. The plan includes the complete file content below so the implementer has zero ambiguity — copy it verbatim.
...
| File | Change |
|------|--------|
| `PROMPTS.md` | Create at repo root (same level as `CLAUDE.md`) |

---

### Task 1: Write PROMPTS.md

**Files:**
- Create: `PROMPTS.md` (repo root — `C:\Users\manoj\New Specter\PROMPTS.md`)

- [ ] **Step 1: Verify repo root contents**

Run:
```powershell
ls "C:\Users\manoj\New Specter" | Select-Object Name
```
Expected: `CLAUDE.md` is present. `PROMPTS.md` is NOT present yet.

- [ ] **Step 2: Create PROMPTS.md with the full content below**

Create `PROMPTS.md` at the repo root (`C:\Users\manoj\New Specter\PROMPTS.md`) with exactly the following content:
...
**Key requirements:**
- `merchants` table: `plan VARCHAR` accepting `'recon' | 'cipher' | 'phantom' | 'predator' | 'eclipse'`; `trial_ends_at TIMESTAMP`
...
- `scrape:ai-errors` queue entry fields: `merchant_id`, `batch_size`, `error_type`, `timestamp`
...
- `specter-web/lib/api.ts` — TanStack Query hooks for all API endpoints (merchants, SKUs, competitors, signals, alerts)
...
- Revenue recovered MTD: `SUM(price_changes.revenue_delta WHERE source='auto')` — F6 AC#2
...
Run:
```powershell
Test-Path "C:\Users\manoj\New Specter\PROMPTS.md"
```
Expected: `True`

Run:
```powershell
(Get-Content "C:\Users\manoj\New Specter\PROMPTS.md" | Measure-Object -Line).Lines
```
Expected: a number greater than 400 (the file has 18 prompts — it's substantial)

- [ ] **Step 4: Verify all 18 prompts are present**

Run:
```powershell
Select-String -Path "C:\Users\manoj\New Specter\PROMPTS.md" -Pattern "^## Prompt \d+:" | Measure-Object | Select-Object -ExpandProperty Count
```
Expected: `18`
...
```powershell
git add PROMPTS.md
git commit -m "feat: add PROMPTS.md — 18-prompt development roadmap for building SPECTER from scratch"
```
...
|---|---|
| File location: repo root | ✅ Task 1 creates `PROMPTS.md` at root |
| Prompt template (Phase/Repo/Read first/Builds on/context/task/deliverables/requirements/success) | ✅ All 18 prompts follow template exactly |
| File intro section (4-6 lines, how-to-use, run-in-order rule) | ✅ Intro section at top of PROMPTS.md content |
| Prompt 1 "Starting point" (no Builds on reference) | ✅ Prompt 1 says "Starting point" |
| Phase 0 (Prompts 1–2) | ✅ Scaffolding + DB Schema |
| Phase 1 (Prompts 3–5) | ✅ Hero/sections, auth pages, 6 tools |
| Phase 2 (Prompts 6–9) | ✅ BullMQ, probe, HTTP, Playwright workers |
| Phase 3 (Prompts 10–11) | ✅ Rule-based + AI signal engines |
| Phase 4 (Prompts 12–14) | ✅ Auth/gates, dashboard pages, repricing/attribution |
| Phase 5 (Prompts 15–18) | ✅ Billing, enterprise features, deployment, CI/CD |
| Cross-cutting rules | ✅ Embedded in relevant prompts (never commit .env, no API from tools, plan gating server-side) |
| One file, one commit | ✅ Single task, single commit |

**Placeholder scan:** No TBDs, TODOs, or vague steps found. Every prompt has exact file paths, exact formulas, exact field names.

**Consistency check:** Types and names used across prompts are consistent (e.g., `ScrapeJob` introduced in Prompt 6 matches what Prompt 7 builds upon; `parseHtml` introduced in Prompt 8 matched in Prompt 9's dual-export requirement; `source='rule'` in Prompt 10 matches schema defined in Prompt 2).
