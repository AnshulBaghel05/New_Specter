...
- Proxies: Bright Data ISP residential (Playwright) + datacenter (probe/HTTP)
...
| `scrape:playwright` | Scrape JS_REQUIRED domains | High (Chromium) | Bright Data ISP residential |
...
// Dead-letter and ops queues — must be instantiated explicitly; BullMQ does not auto-create them
export const deadLetterQueue = new Queue('scrape:dead-letter', { connection: redis })
export const validationErrorsQueue = new Queue('scrape:validation-errors', { connection: redis })
```
...
// Note: plan values in ScrapeJob are lowercase ('recon', not 'RECON').
// The BullMQ priority table uses ALLCAPS for readability only.
// Priority lookup: PLAN_PRIORITY[job.plan.toUpperCase()] or use a map.
```
...
2. If lock exists: merge `competitorTrackingIds` into the existing job's array (no new job created, no duplicate outbound request)
3. If no lock: create new job, set lock with TTL = plan's scrape interval

One outbound HTTP request serves all merchants whose `competitor_trackings` reference the same `competitor_url`. specter-api receives the full `competitor_tracking_ids[]` array in the price-snapshot POST and fans out signals/OOS alerts to each affected merchant independently.
...
1. `robots.txt` disallows URL path → **BLOCKED** (robots.txt cached in Redis key `robots:{domain}`, TTL 24hr — see robots.txt Compliance section for full flow)
...
| Parameter | Value |
|-----------|-------|
| Concurrency | 50 per worker process (I/O-bound, no browser) |
...
7. If BLOCKED via bot wall (step 2): classify BLOCKED, POST to specter-api `/internal/domain-blocked` with `competitor_tracking_ids[]` and `robots_blocked: false`. Note: `robots_blocked` is NOT set on the competitor_url record for bot-wall blocks — only for robots.txt blocks (see robots.txt Compliance section).
   If BLOCKED via robots.txt (heuristic 1): classify BLOCKED, POST to specter-api `/internal/domain-blocked` with `competitor_tracking_ids[]` and `robots_blocked: true`. specter-api sets `competitor_urls.robots_blocked = true` and sends merchant Resend email.
...
5. If valid → write to `price_snapshots`, POST to specter-api `/internal/price-snapshot` with `competitor_tracking_ids[]` so specter-api fans out signals per merchant, DEL `domain:http-fail:{domain}` counter (reset to 0 on any successful parse)
...
// Playwright worker: receives a live browser page
export async function parse(page: Page): Promise<ParseResult>

// HTTP worker: receives raw HTML string (sync, no browser)
...
Both HTTP and Playwright workers produce `ParseResult`. Playwright workers call `parse(page)` with a live browser `Page`. HTTP workers call `parseHtml(html)` with the raw HTML string — this variant is synchronous (no browser, no async needed). Both functions are exported from each domain parser file.
...
| Playwright | Residential | Bright Data ISP residential | $8.40 |
...
3. If disallowed: classify BLOCKED, set `competitor_urls.robots_blocked = true`, POST `/internal/domain-blocked` with `competitor_tracking_ids[]`, notify merchant via Resend
