...
due URLs every `DISPATCH_TICK_SECONDS` and enqueues one shared crawl each.
...
**Verify:** each worker logs `worker started` (e.g. `[http] worker started —
concurrency 30`). End-to-end check: add a competitor in the dashboard (§11) and
...
**Verify:** `curl -fsS -X POST …/internal/run-trial-monitor -H "Authorization:
Bearer $TRIAL_MONITOR_SECRET"` → `{"status":"ok",…}`. A `401` = wrong secret; a
